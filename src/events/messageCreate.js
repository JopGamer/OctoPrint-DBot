...
new Listener({
    ...
    run: async(message) => {
        ...
        const psu = await (await fetch(`${config.octoprint.url}/api/plugin/psucontrol`, {
            method: 'POST',
            headers: {
        ...
        setTimeout(async() => {
            const embed = new MessageEmbed()
                .setColor('BLUE')
                .setDescription(`**File is uploading...**`);
            ...
            const embed3 = new MessageEmbed()
                .setColor('GREEN')
                .setDescription(
                    `Printing of file **${message.attachments.first().name}** has been started!`,
                );
